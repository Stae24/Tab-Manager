...
import { Plus, Sun, Moon, ZoomIn, ZoomOut, RefreshCw, Download, Settings, X, Save, ChevronDown } from 'lucide-react';
import { useStore } from '../store/useStore';
import { cn } from '../utils/cn';

export const Sidebar: React.FC = () => {
  const { isDarkMode, toggleTheme, uiScale, setUiScale, islands, vault, showVault, setShowVault } = useStore();
  ...
  return (
    <>
      <div className="flex flex-col gap-4 p-4 border-b border-gx-gray bg-gx-dark">
        {/* Logo / Title */}
        <div className="flex items-center justify-between">
          ...
          <button 
            ...
            className="p-2 rounded-lg bg-gx-gray hover:bg-gx-accent/20 transition-all text-gray-400 hover:text-white"
          >
            <Settings size={18} />
          </button>
        </div>
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[100] flex items-center justify-center p-6">
          <div className="w-full max-w-md bg-gx-dark border border-gx-gray rounded-2xl p-6 shadow-2xl relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-gx-accent via-gx-red to-gx-accent animate-pulse" />
            
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-xl font-black text-white tracking-tighter italic uppercase flex items-center gap-3">
                <Settings className="text-gx-accent" /> Preferences
              </h2>
              <button 
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-4 bg-gx-gray/30 p-4 rounded-xl border border-white/5">
                  <button onClick={() => setUiScale(Math.max(0.75, uiScale - 0.05))} className="p-2 rounded-lg bg-gx-gray hover:bg-gx-accent/20 transition-all"><ZoomOut size={16} /></button>
                  ...
                  <button onClick={() => setUiScale(Math.min(1.5, uiScale + 0.05))} className="p-2 rounded-lg bg-gx-gray hover:bg-gx-accent/20 transition-all"><ZoomIn size={16} /></button>
                </div>
              </div>

              <div className="pt-4 border-t border-gx-gray">
                <p className="text-[10px] text-gray-600 font-mono text-center uppercase tracking-widest">
                  Island Manager v1.0.0
                </p>
              </div>
            </div>
          </div>
